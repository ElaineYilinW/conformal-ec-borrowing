...
这是一个工作定位,尚非最终的新颖性主张。普通的残差秩选择性借用已由 \citet{zhu2025csb} 发展;协变量偏移下的加权共形校准也已在共形文献中发展 \citep{tibshirani2019weighted,jin2026weighted}。可能的贡献在于它们在外部对照情形中的纵向、估计目标感知(estimand-aware)的整合,以及对该筛选到底诊断与不诊断哪一种兼容性概念的细致研究。
...
不应宣称任何得分普适最优。主分析可使用估计目标对齐的得分,并以最大或马氏得分作为更宽泛的轨迹层面敏感性分析。
...
\begin{itemize}
    ...
    \item 哪个得分检出污染、检出是否与估计目标对齐? --- $\Delta=3$(弱信号看 $\Delta=1$),形态 A--D,按得分形状看检出。
...
\begin{itemize}
    ...
    \item 估计目标对齐对比 $c_1^{\top}\bm r$ 与 $c_2^{\top}\bm r$(带符号,每个估计目标一个)。
\end{itemize}
三种协方差处理设定 $A$ 与 $a_t$:
\begin{itemize}
    \item 原始($A=I$,$a_t=1$);
...
\begin{table}[ht]
\centering
\caption{估计,每格为\emph{偏差(覆盖率)}($n_{\mathrm{sim}}=400$,百分位 CI 来自 $B=200$ 次整流程自助;加权 CV+ 筛选,估计目标对齐的内部标准化对比,正确模型,$\gamma=0.10$,$\pi_B=0.5$)。真值 $\tau_{c_1}=1.000$、$\tau_{c_2}=0.4625$。空场景为 $\Delta=0$;晚期漂移 B 与与 $c_1$ 正交的平移 A 在 $\Delta=3$。``单边''/``双边''为单侧/双侧对比筛选。}
...
\begin{enumerate}
    ...
    \item \textbf{得分要对准估计目标。} 与估计目标对齐的对比得分,只对偏倚\emph{该}估计目标的污染报警、并正确忽略与之正交的漂移;综合(全轨迹)得分则什么都报警、把对目标无害的 EC 也多删。两个估计目标下,同一漂移偏其一不偏其二,而对齐得分只看相关的那个。
