...
\large Illustrated Detailed Research Blueprint --- New Version}
\author{}
\date{July 2026}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textcolor{midgray}{Longitudinal residual-rank screening}}
\fancyhead[R]{\small\textcolor{midgray}{Illustrated detailed blueprint}}
\fancyfoot[C]{\textcolor{midgray}{\thepage}}
\renewcommand{\headrulewidth}{0.3pt}

\begin{abstract}
This document develops and illustrates a working method for selective borrowing of longitudinal external controls. Randomized controls define an empirical reference distribution for an estimand-aware residual-trajectory score; covariate-shift weights make that reference represent the compatible external covariate mix; and a symmetric two-sided screen avoids the orientation bias created by deleting only one clean outcome tail. The practical implementation uses cross-validated scores so every scarce randomized control contributes to calibration. After screening, all source, outcome, variance, transport, and borrowing components are refit before a transported AIPW control mean is combined with the randomized estimate. The document separates causal identification from the weaker compatibility object diagnosed by a chosen scalar score, compares covariance standardizations and score shapes, and reports a structured simulation program. In the current reference cell, the symmetric adaptive screen is unbiased, controls type-I error, and improves RMSE and power relative to RCT-only while approaching an unattainable oracle. The gain is deliberately framed as insurance against unsafe full borrowing, not as a universal or large efficiency improvement; weighted CV+ validity, adaptive post-selection inference, and realistic cross-scenario validation remain open.
\end{abstract}

\begin{figure}[!ht]
\centering
\fullfig{problem_landscape.tex}
\caption{The statistical decision in one picture. The randomized treated arm always estimates the treated mean; the methodological question is how much external information may safely supplement the randomized control mean.}
\label{fig:problem-landscape}
\end{figure}

\begin{takeaway}
This blueprint is easiest to read as two linked procedures: first a subject-level compatibility diagnostic; then a newly refit causal estimator on the retained external controls. The two stages use different density ratios in opposite directions.
\end{takeaway}
...
\begin{figure}[!ht]
\centering
\fullfig{two_gates.tex}
\caption{Baseline support and outcome compatibility are separate gates. Propensity-based tools address the first; the proposed residual-rank diagnostic addresses the second.}
\label{fig:two-gates}
\end{figure}

For a non-specialist, Figure~\ref{fig:two-gates} prevents the most common conceptual error: covariate balance does not guarantee comparable control outcomes, and a plausible outcome trajectory does not repair a lack of baseline support. Outcome screening begins only after eligibility, measurement, and support have been harmonized.
...
\begin{figure}[!ht]
\centering
\fullfig{compatibility_ladder.tex}
\caption{Compatibility targets and the claim boundary. A scalar score is a deliberately lossy diagnostic summary; passing its screen is not evidence for every stronger notion above it.}
\label{fig:compatibility-ladder}
\end{figure}
...
\begin{figure}[!ht]
\centering
\fullfig{score_pipeline.tex}
\caption{From an observed longitudinal record to a borrowing decision. The working model removes the expected covariate-specific course and defines a useful ordering; out-of-fold randomized controls empirically calibrate how extreme that ordering is.}
\label{fig:basicrank}
\end{figure}
...
Figure~\ref{fig:basicrank} communicates the key idea to a wider audience: the outcome model supplies a useful ordering, while rank calibration determines whether the external observation is unusually incompatible relative to actual randomized controls. The scalar score is a purposeful compression of the trajectory, not a replacement for prespecifying the clinical estimand.
...
\begin{figure}[!ht]
\centering
\fullfig{drift_score_alignment.tex}
\caption{Score choice is a scientific design choice. A contrast score is intentionally sensitive to drift that projects onto its estimand and can be blind to orthogonal drift; an omnibus score sees more trajectory differences but can exclude controls that are harmless for the target mean contrast.}
\label{fig:score-alignment}
\end{figure}
...
\begin{figure}[!ht]
\centering
\fullfig{covariance_targets.tex}
\caption{Common-reference and source-specific standardization answer different questions. The latter can intentionally normalize away a pure covariance difference, but its estimated external covariance can also absorb contamination.}
\label{fig:covariance-targets}
\end{figure}
...
&\text{or adequate two-sided overlap on the target region}.
\end{aligned}
\tag{8}
\]
and
...
\begin{figure}[!ht]
\centering
\fullfig{covariate_shift.tex}
\caption{The weighting mechanism in the base DGP. The compatible conditional outcome law may be identical even though the two marginal score mixtures differ; weighting changes each randomized score's mass so the reference has the compatible-EC covariate mix.}
...
\begin{figure}[!ht]
\centering
\fullfig{split_vs_cv.tex}
\caption{Why CV+ is used in practice. A single split has the cleanest finite-sample foundation but partitions a scarce randomized-control sample; cross-fitting gives every control an out-of-fold score and allows every control to calibrate ranks.}
\label{fig:split-vs-cv}
\end{figure}
...
\begin{figure}[!ht]
\centering
\fullfig{symmetry_bias.tex}
\caption{Why the carried-forward default is symmetric. When the score is aligned with the estimand, asymmetric deletion of clean scores changes the retained clean outcome mean and therefore biases the treatment-effect estimate.}
\label{fig:symmetry-bias}
\end{figure}
...
&=\max\!\left[\widehat D_\gamma^2-
\widehat{\mathrm{Var}}^{\ast}(\widehat D_\gamma),\,0\right]
+\widehat{\mathrm{Var}}^{\ast}\!\{\widehat\tau(\gamma)\},\\
\gamma^{\ast}
&=\arg\min_{\gamma\in\mathcal G}\widehat{\mathrm{MSE}}(\gamma).
\end{aligned}
\]
...
\begin{figure}[!ht]
\centering
\fullfig{screen_refit_workflow.tex}
\caption{The complete workflow and the density-ratio reversal. Screening reweights randomized scores toward the compatible external mix; estimation transports retained external outcomes back toward the randomized-trial mix.}
\label{fig:screen-refit}
\end{figure}
...
\begin{figure}[!ht]
\centering
\fullfig{simulation_map.tex}
\caption{The simulation program moves from an analytically transparent mechanism study to a more general covariate and estimator structure. It is designed to isolate one failure mode at a time rather than to mimic a clinical dataset immediately.}
...
\begin{figure}[!ht]
\centering
\fullfig{results_dashboard.tex}
\caption{Reference-cell result as a decision dashboard. The symmetric adaptive default is the only attainable screen that clears the four prespecified gates simultaneously; the oracle is shown only as an unattainable ceiling. Exact values and additional score variants remain in Table~\ref{tab:refcell}.}
\label{fig:reference-dashboard}
\end{figure}

The visual message in Figure~\ref{fig:reference-dashboard} is deliberately stricter than ranking methods by power alone. A method must first avoid material bias and type-I inflation; only then do RMSE and power gains represent valid efficiency. This is why the high power of the non-symmetric alternatives is not treated as a success.
...
\[
\begin{aligned}
X_b\mid S{=}1&\sim\mathrm{Bernoulli}(0.30),
&X_b\mid S{=}0&\sim\mathrm{Bernoulli}(0.70),\\
X_c\mid S{=}1&\sim N(0,1),
&X_c\mid S{=}0&\sim N(0.6,1).
\end{aligned}
\]
...
A practical order of work from the current state is:
\begin{enumerate}
    \item pressure-test the carried-forward symmetric adaptive screen across patterns B/C/D, contamination fractions $\pi_B\in\{0.15,0.30,0.50\}$, and a signal-to-noise grid;
    \item repeat adaptive threshold selection inside each bootstrap or develop a cheaper nested approximation, so uncertainty assessment reflects threshold selection;
    \item evaluate a bias-neutral soft or continuous borrowing rule that can use directional evidence without truncating one clean outcome tail;
    \item add $X$-dependent contamination, explicitly separating the observable overall external density ratio from the latent compatible-external ratio;
    \item regularize the common-reference covariance and assess whether any omnibus-score gain survives small randomized-control samples;
    \item move to a covariate-driven plasmode design with realistic longitudinal missingness and differences in the observation process;
    \item develop theory for the weighted longitudinal CV+ statistic and for post-screen refitted AIPW inference.
\end{enumerate}

This order preserves the current evidence boundary: broaden empirical support first, then strengthen inference and theory, and only afterward make a realistic application claim.
