...
Figure~\ref{fig:mech} shows one simulated draw. On the raw outcome (top) the compatible ECs (blue) overlap the randomized controls (gray) but sit slightly higher \emph{purely because the external mix carries more severe subjects}---covariate shift, not incompatibility---while the contaminated ECs (amber) sit far right; a raw-outcome threshold would confound the two. The nonconformity score (bottom) subtracts the covariate mean, so randomized and compatible controls realign at $0$ and only contamination stands out. The symmetric screen keeps the central band, borrowing the compatible controls and dropping the incompatible ones; trimming both tails equally leaves the retained mean at $0$, hence unbiased.

\begin{figure}[ht]\centering
\includegraphics[width=.92\linewidth]{fig_outcome.pdf}\\[2pt]
\includegraphics[width=.92\linewidth]{fig_score.pdf}
\caption{Outcome (top) vs.\ nonconformity score (bottom) for randomized controls, compatible ECs (borrowed), and non-compatible ECs (dropped). The score removes covariate shift; the symmetric keep band borrows the compatible ECs unbiasedly.}
\label{fig:mech}
\end{figure}
...
\caption{Reference cell (base binary DGP, $\pi_B{=}0.3$, pattern~A, $\Delta{=}8$, visit-average estimand, $\tau{=}0.4625$; weighted CV+ rank calibration, correct model; $300$ replicates). sym-ada beats RCT-only on all four criteria at once (unbiased; RMSE $0.18$ vs.\ $0.20$; type-I no worse than RCT-only, $0.08$; power $0.75$ vs.\ $0.65$), approaching the oracle. The non-symmetric screens' higher power is a bias/type-I artifact; full borrowing is catastrophic.}
\label{tab:ref}
\begin{tabular}{lcccccc}
\toprule
Method & Detection & False-excl. & Bias & RMSE & Type-I & Power \\
\midrule
RCT-only            & --- & --- & $-0.00$ & $0.20$ & $0.08$ & $0.65$ \\
Full EC-AIPW        & --- & --- & $-0.42$ & $0.49$ & $0.46$ & $0.08$ \\
Oracle              & $1.00$ & $0.00$ & $-0.00$ & $0.17$ & $0.07$ & $0.78$ \\
\midrule
\textbf{sym-ada (default)} & $1.00$ & $0.10$ & $0.00$ & $\mathbf{0.18}$ & $0.08$ & $\mathbf{0.75}$ \\
nonsym-ada          & $0.98$ & $0.07$ & $+0.05$ & $0.21$ & $0.10$ & $0.83$ \\
nonsym-fix          & $1.00$ & $0.32$ & $+0.31$ & $0.36$ & $0.35$ & $0.99$ \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{CV+ over split.} Cross-fitting the calibration lifts power by $0.07$--$0.09$ and roughly doubles the RMSE gain, at controlled type-I, by not wasting the randomized controls.
...
\begin{table}[ht]\centering
\caption{One-axis robustness off the sym-ada/CV+ baseline (hetero base DGP, $\pi_B{=}0.3$, $\Delta{=}8$; $300$ replicates). Each row changes one thing.}
\label{tab:robust}
\begin{tabular}{lcccccc}
\toprule
Setting & Bias & RMSE & Type-I & Power & Detection & False-excl. \\
\midrule
baseline (sym-ada, CV+) & $-0.00$ & $0.18$ & $0.06$ & $0.74$ & $0.99$ & $0.11$ \\
severe model misspec.\  & $+0.00$ & $0.19$ & $0.07$ & $0.74$ & $1.00$ & $0.11$ \\
unweighted (correct)    & $+0.00$ & $0.18$ & $0.06$ & $0.76$ & $1.00$ & $0.10$ \\
split instead of CV+    & $-0.00$ & $0.19$ & $0.07$ & $0.67$ & $0.99$ & $0.12$ \\
C-slope drift, $c_2$    & $-0.02$ & $0.19$ & $0.05$ & $0.66$ & $0.88$ & $0.14$ \\
C-slope drift, Mahalanobis & $-0.02$ & $0.19$ & $0.06$ & $0.73$ & $0.94$ & $0.19$ \\
C-slope drift, max (trap)  & $+0.05$ & $0.22$ & $0.13$ & $0.82$ & $0.88$ & $0.07$ \\
\bottomrule
\end{tabular}
\end{table}
